...
from known_keys import KNOWN_KEYS, KNOWN_SIGNATURES
from xcryptors import Xcryptor, CBCXcryptor
from known_keys import mac_to_str
...
def decrypt(infile, decryptor, keypair):
    ...
    if decrypted is not None:
        if zte.read_payload_type(decrypted, raise_on_error=False) is not None:
            return decrypted
    return None
...
def main():
    ...
    infile.seek(0)
    zte.read_header(infile, little_endian=args.little_endian)

    signature = zte.read_signature(infile).decode()
    if args.signature is None:
        args.signature = signature

    payload_type = zte.read_payload_type(infile)
    if payload_type != 0:
        ...
        decompressed, _ = compression.decompress(infile)
        args.outfile.write(decompressed.read())
        print(f"Successfully decompressed {infile.name}")
        return 0

    decompressed, _ = compression.decompress(decrypted)
    args.outfile.write(decompressed.read())
    print(
